...
def build_section_chunks(profile: dict, bib: dict, bio_md: str, news_md: str,
    ...
    return {
        ...
        "LOCATION_HTML": build_location_html(profile),
        ...
        "ACTIVITIES_SECTION_HTML": build_activities_section(profile),
...
def build_location_html(profile: dict) -> str:
    """Render an optional plain-text location from identity.location."""
    location = (profile.get("identity", {}) or {}).get("location")
    return esc(location) if location else ""
...
def build_research_experiences_section(profile: dict) -> str:
    """Render current and past research roles as a detailed timeline."""
    affiliations = profile.get("affiliations", {}) or {}
    current = affiliations.get("current", []) or []
    past = affiliations.get("past", []) or []
    experiences = [(item, True) for item in current] + [(item, False) for item in past]
    if not experiences:
        return ""
    rows = []
    for a, is_current in experiences:
        role = esc(a.get("role", ""))
        inst = esc(a.get("institution", ""))
        dept = esc(a.get("department", "")) if a.get("department") else ""
        location = esc(a.get("location", "")) if a.get("location") else ""
        ...
        topics_html = "".join(f"<li>{esc(topic)}</li>" for topic in topics)
        current_html = '<span class="experience-current">Current</span>' if is_current else ""
        summary_html = f'<p class="experience-summary">{md_inline(str(summary))}</p>' if summary else ""
        header_html = (
            '<article class="experience-item">'
            '<div class="experience-header">'
            f'<div><h3>{role}</h3><p class="experience-institution">{inst}</p></div>'
            f'<div class="experience-date">{current_html}<span>{esc(dates)}</span></div>'
            '</div>'
        )
        if meta:
            header_html += f'<p class="experience-meta">{meta}</p>'
        rows.append(header_html)
        if topics_html:
            rows[-1] += f'<ul class="experience-topics">{topics_html}</ul>'
        rows[-1] += summary_html + '</article>'
    return (f'<section class="experience"><h2>Research Experience</h2>\n'
            f'<div class="experience-list">{"".join(rows)}</div>\n</section>')


def build_activities_section(profile: dict) -> str:
    """Render selected academic and technical activities."""
    ...
    for item in activities:
        ...
        rows.append(
            ...
            f'<span class="activity-year">{year}</span>'
            f'<span><strong>{name}</strong>{" - " + detail if detail else ""}</span>'
            '</div>'
        )
    return (f'<section class="activities"><h2>Selected Activities</h2>\n'
            f'{"".join(rows)}\n</section>')
...
def render_template(template: str, chunks: dict[str, str], profile: dict,
    ...
    base_vars = {
        ...
        "PAGE_TITLE": f"{profile['identity']['name']} | Academic Homepage",
        "META_DESCRIPTION": esc(profile.get("research", {}).get("summary", "")),
        "CANONICAL_URL": esc(profile.get("links", {}).get("homepage", "")),
